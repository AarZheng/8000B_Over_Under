...
void Drive::drive_distance(float distance, float heading, float drive_max_voltage, float heading_max_voltage, float drive_timeout){
  drive_distance(distance, heading, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}
...
void Drive::control_arcade(){
  ...
  float controllerTurn = deadband(controller(primary).Axis1.value(), 5);
  float turn = controllerTurn * controllerTurn / 127;
  if(controllerTurn < 0) {
    turn = -turn;
  }
  DriveL.spin(fwd, to_volt(throttle+(0.8*turn)), volt);
  DriveR.spin(fwd, to_volt(throttle-(0.8 * turn)), volt);
}
